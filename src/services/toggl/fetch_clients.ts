// fetchClients.ts - Toggl API v9からクライアント情報を取得

import "https://deno.land/std@0.203.0/dotenv/load.ts";
import { TogglApiV9Client } from "./types.ts";

// --- Environment variables ---
const API_TOKEN = Deno.env.get("TOGGL_API_TOKEN")?.trim();
const WORKSPACE_ID = Deno.env.get("TOGGL_WORKSPACE_ID")?.trim();

if (!API_TOKEN || !WORKSPACE_ID) {
  throw new Error("TOGGL_API_TOKEN or WORKSPACE_ID is not set in .env");
}

// --- Authentication header ---
const authHeader = {
  "Content-Type": "application/json",
  "Authorization": `Basic ${btoa(`${API_TOKEN}:api_token`)}`,
};

// --- Rate limiting utility ---
const delay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

/**
 * Fetch all clients from Toggl workspace
 * @returns Array of clients
 */
export async function fetchClients(): Promise<TogglApiV9Client[]> {
  const url = `https://api.track.toggl.com/api/v9/workspaces/${WORKSPACE_ID}/clients`;
  
  const res = await fetch(url, { headers: authHeader });
  
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Failed to fetch clients: ${res.status} ${res.statusText}\n${text}`);
  }
  
  const clients: TogglApiV9Client[] = await res.json();
  return clients;
}

/**
 * Fetch clients with retry logic
 * @param maxRetries Maximum number of retry attempts
 * @param retryDelay Delay between retries in milliseconds
 * @returns Array of clients
 */
export async function fetchClientsWithRetry(
  maxRetries: number = 3,
  retryDelay: number = 2000
): Promise<TogglApiV9Client[]> {
  let lastError: Error | null = null;
  
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      return await fetchClients();
    } catch (error) {
      lastError = error as Error;
      
      if (attempt < maxRetries) {
        await delay(retryDelay);
      }
    }
  }
  
  throw new Error(`Failed to fetch clients after ${maxRetries} attempts: ${lastError?.message}`);
}