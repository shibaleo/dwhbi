...
| **project_name** | TEXT | Togglプロジェクト名 | |
| **project_color_hex** | TEXT | Togglプロジェクト色（HEX形式） | |
...
| フィールド | 用途 |
|------------|------|
| project_name | ダッシュボードでのプロジェクト別集計、フィルタリング |
| project_color_hex | UI での色分け表示、Toggl と一貫した視覚表現 |

## ソース

```
stg_toggl_track__time_entries
  → stg_toggl_track__projects (project_id)
      ├── project_name
      ├── color (→ project_color_hex)
      └── client_id → stg_toggl_track__clients
                        └── client_name
```

### データ取得元

| フィールド | ソーステーブル | ソースカラム |
|------------|----------------|--------------|
| project_name | stg_toggl_track__projects | project_name |
| project_color_hex | stg_toggl_track__projects | color |
| time_category_personal | map_toggl_color_to_time_personal | time_category_personal |
| time_category_social | map_toggl_client_to_time_social | time_category_social |
