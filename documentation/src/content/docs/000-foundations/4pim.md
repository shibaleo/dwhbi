...
4PIM（4 Practice-Information Model）は、個人・組織の行動の結果として計測できるデータの活用を構造化するモデルである。名称の「Practice-Information」は、4つの実践（practices）が4つの情報（information）を生むという関係性を表す。

4PIMは「こうすべき」という規範を示すフレームワークではなく、「データ活用においてどのような情報が存在しうるか」を記述するモデルである。すべてのパターンを実践する必要はなく、状況に応じて選択できる。
...
**対象とするデータ**
- 主体（個人または組織）の行動に起因し、その行動を改善可能なデータを対象とする。

**対象外のデータ**
- 天気など、観測のみで改善の余地がないデータ。4PIMは「主体の行動による改善可能性」を前提としたモデルである。
...
logの実態は「測定」であり、手入力または自動計測によって行動や状態を記録する。

- 人間が測定することによる「真の値からのズレ」は考慮せず、記録された文字列をactualとして扱う。
- targetはlogの「入力」ではなく、人間が行動する際の「指針」である。
- targetとactualの間には必ず「人間の行動」という断絶がある。
- 「やる気がなければデータに基づいた行動改善はできない」という真実を前提に置きつつ、モデルはその断絶を前提に構造化されている。

### analyze（分析）

actualを集計・分析し、estimateを導出する。

- actualだけでなく、客観的外部データ（objective inputs）も入力となる。例：マスタデータ、気象情報、機械学習モデルのハイパーパラメータなど。
- 主観を含まない客観的データのみを扱う。

### aim（目標草案の設定）

**aim は estimate を基に、主観的外部要因（subjective external factors）を取り込み、主観的な未来像である draft を生成する内的プロセスである。**

- estimateはdraftを考えるための材料となる。
- 主観的外部要因は、analyze の客観的外部データ（objective inputs）とは異なり、主体の外部から与えられる目的・制約・願望といった外生的な入力である。例：「今年中に資格を取りたい」「上司から月100時間の指示」「家族との時間を確保したい」など。
...
- draft が「主観的な理想」であるのに対し、target は「実現可能性に制約された主観」である。
...
回答：当然変わりうるが、目標の変更は行動後の目標の再考慮を意味する。一度目標を決めた後は、その目標を前提として行動し、記録するという過程を説明するために「主観/確実」と定義している。
...
| 3 | **draft** | 理想・願望を含む草案（不確実） | 「来月は100時間働きたい」 |
| 4 | **target** | draftを現実制約で調整した目標 | 「85時間が妥当」 |
...
4PIMは4つの情報すべてを揃えることを強制しない。actualを必須とすると、残り3つ（estimate, draft, target）の有無により8パターンが存在する。
...
| 草案設定 | actual + estimate + draft | 推定を参照して草案を設定するが、調整はしないので目標達成の意識が薄れる |
...
| フル活用 | actual + estimate + draft + target | 4practicesすべてが機能し、継続的な改善が可能 |
...
に活用できる。target を明示的に記録することで、モデル全体の精度が経験的に向上する。
...
4PIMは独自の概念ではなく、既存のフレームワークや概念と共通点がある。

| 概念 | 共通点 |
|------|--------|
| **PDCA** | 反復的な改善構造（Plan-Do-Check-Act）。PDCAは行動の循環を前提とするが、4PIMは情報の分類を主眼とし、「サイクルを回す」ことを前提としない |
...
これらの概念に馴染みがあれば、4PIMの理解の助けとなる。GTDやQuantified Selfは個人向けに設計されたフレームワークだが、4PIMとの共通点は主体を問わない。

4PIMは独自の新理論ではなく、DIKW、目標設定理論、自己調整理論、Quantified Selfなど既存の個別理論を「情報の種類」という観点から統合・再整理した記述モデルである。
...
4PIMは既存の個別理論の一部分を統合し、“情報の種類”として再整理した記述モデルである
