...
| 機能 | 理由 |
|------|------|
| target（目標値）の調整 | LLMとの対話で行う |
| plan（計画データ）の調整 | LLMとの対話、または外部サービス（Google Calendar等）で行う |
...
- Core層: ビジネスロジック適用済み・サービス統合済みデータ（actual, plan, estimate等）
...
実績データ（actual）を基に、推定値（estimate）を自動算出する。
