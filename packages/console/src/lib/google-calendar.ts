/**
 * Google Calendar Event Creation for Console
 *
 * Credentials are retrieved via Supabase RPC (vault).
 */
...
// Cache for auth (per-request, cleared on each server restart)
...
async function getAuthInfo(forceRefresh: boolean = false): Promise<AuthInfo> {
  ...
  // Load from vault via Supabase RPC
  ...
  // Parse expires_at from _expires_at metadata (already stripped by getServiceCredentials)
  ...
  if (rawData?._expires_at) {
    expiresAt = new Date(rawData._expires_at);
  }
  ...
  if (needsRefresh) {
    ...
    // Update vault via Supabase RPC
    await updateServiceCredentials("google_calendar", {
      access_token: accessToken,
      scope: newToken.scope,
      _expires_at: currentExpiresAt.toISOString(),
    });
...
function buildEventBody(event: CreateEventInput): Record<string, unknown> {
  ...
  if (event.extendedProperties) {
    ...
    if (ep.toggl_track_project_id)
      privateProps.toggl_track_project_id = ep.toggl_track_project_id;
    if (ep.pattern_group_id)
      privateProps.pattern_group_id = ep.pattern_group_id;
    if (ep.registered_at) privateProps.registered_at = ep.registered_at;
    if (ep.tags && ep.tags.length > 0)
      privateProps.tags = JSON.stringify(ep.tags);
...
function buildBatchRequest(
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    parts.push("");
    parts.push(
      `POST /calendar/v3/calendars/${calendarIdEncoded}/events HTTP/1.1`
    );
  ...
  for (const part of parts) {
    ...
    if (statusCode >= 200 && statusCode < 300 && jsonMatch) {
      try {
        ...
        results.push({
          event,
          error: `Failed to parse response: ${jsonMatch[1]}`,
        });
      }
    } else {
...
async function executeBatch(
  ...
  Array<{ event: CreateEventInput; result?: CreateEventResult; error?: string }>
> {
...
export async function createEvents(
  ...
  created: CreateEventResult[];
  failed: { event: CreateEventInput; error: string }[];
}> {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    try {
      ...
      if (message.includes("401")) {
        ...
        try {
          const retryResults = await executeBatch(
            batch,
            targetCalendarId,
            newAuth.accessToken
          );
